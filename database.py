...
class Usuario(db.Model):
    ...
    llamadas    = db.relationship('Llamada',    backref='usuario', lazy=True, cascade='all, delete-orphan')
    propietarios= db.relationship('Propietario',backref='usuario', lazy=True, cascade='all, delete-orphan')
    ...
    def puede_crear_lead(self):
        ...
        if not self.activo: return False, 'Tu cuenta está desactivada.'
        if self.periodo_prueba and self.prueba_expira and datetime.now() > self.prueba_expira:
            return False, 'Tu período de prueba venció. Contactá al administrador.'
    ...
    def dias_prueba_restantes(self):
        if self.periodo_prueba and self.prueba_expira:
            return max(0, (self.prueba_expira - datetime.now()).days)
        return 0
...
class Visita(db.Model):
    ...
    def estado_label(self):
        if self.realizada: return 'Realizada'
        ahora = datetime.now()
        if self.fecha_hora < ahora: return 'Pendiente confirmar'
        diff = self.fecha_hora - ahora
        dias = diff.days
        if dias == 0: return f'Hoy a las {self.fecha_hora.strftime("%H:%M")}'
        elif dias == 1: return f'Mañana a las {self.fecha_hora.strftime("%H:%M")}'
        else: return f'En {dias} días – {self.fecha_hora.strftime("%d/%m a las %H:%M")}'

    @property
    def urgencia(self):
        if self.realizada: return 'realizada'
        ahora = datetime.now()
        if self.fecha_hora < ahora: return 'vencida'
        diff = (self.fecha_hora - ahora).days
        if diff == 0: return 'hoy'
        elif diff == 1: return 'manana'
        return 'proxima'
...
class Plantilla(db.Model):
    ...
    DEFAULTS = {
        'seguimiento_1':       {'nombre': 'Seguimiento 1 (24hs)',           'texto': 'Hola {nombre}! Fue un gusto mostrarte {propiedad} hoy. Me quede pensando en lo que comentaste. Alguna pregunta que haya surgido?'},
        'micro_contacto':      {'nombre': 'Micro contacto (dia 3-4)',        'texto': 'Hola {nombre}! Te comparto algo sobre la zona de {propiedad}. Viene creciendo en valor. Seguis evaluando la opcion?'},
        'seguimiento_2':       {'nombre': 'Seguimiento 2 (dia 5-7)',         'texto': 'Hola {nombre}! {propiedad} sigue disponible. Podemos charlar 5 minutos esta semana?'},
        'cierre':              {'nombre': 'Cierre (dia 10-14)',              'texto': 'Hola {nombre}! Han pasado unos dias desde que viste {propiedad}. Te parece si coordinamos el proximo paso?'},
        'captacion_inicio':    {'nombre': 'Captacion – Primer contacto',    'texto': 'Hola {nombre}! Te contacto porque tenes una propiedad en {propiedad} que podria interesarte vender. Te gustaria que conversemos?'},
        'captacion_seguimiento':{'nombre': 'Captacion – Seguimiento',       'texto': 'Hola {nombre}! Como estas? Queria retomar la charla sobre {propiedad}. Tenes unos minutos?'},
        'reporte_semanal':     {'nombre': 'Reporte semanal (lunes)',         'texto': 'Hola {nombre}! Te paso el reporte de tu propiedad en {propiedad}. Esta semana tuvimos [X] visitas. Cualquier consulta estoy a disposicion!'},
        'recordatorio_visita': {'nombre': 'Recordatorio de visita (2hs)',   'texto': 'Hola {nombre}! Te recuerdo que en un rato tenemos la visita a {propiedad} a las {hora}. Cualquier inconveniente avisame.'},
    }
...
class PropiedadCaptada(db.Model):
    ...
    ESTADOS = {'captada': 'Captada', 'en_venta': 'En venta', 'reservada': 'Reservada', 'vendida': 'Vendida'}
...
class Llamada(db.Model):
    ...
    ESTADOS = {'pendiente': 'Pendiente', 'completada': 'Completada', 'no_atendio': 'No atendió', 'descartada': 'Descartada'}
    ...
    def minutos_para_llamada(self):
        diff = (self.fecha_hora - datetime.now()).total_seconds() / 60
        return diff
    ...
    def urgencia(self):
        ...
        elif mins <= 10: return 'ahora'
        elif mins <= 60: return 'pronto'
        return 'proxima'

    @property
    def estado_label_tiempo(self):
        mins = self.minutos_para_llamada
        if mins < 0: return 'Llamada pendiente de resultado'
        elif mins <= 10: return f'En {int(mins)} minutos'
        else: return self.fecha_hora.strftime('%d/%m/%Y a las %H:%M')
